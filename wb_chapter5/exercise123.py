...
# Create a punctuation marks list
punctuation_marks = [",", ".", "?", "-", "'", "!", ":", ";"]
...
for i in range(len(words_in_sentence)):
    ...
    # First letter of each word is lowercase by default
    first_letter = "lowercase"
    
    # Each word doesn't have a punctuation mark by default
    is_punctuation_mark = False
    
    # Check if first letter is uppercase; if so,
    # lower it
    if word[0].isupper():
        to_lower_letter = word.pop(0)
        lower_letter = to_lower_letter.lower()
        word.insert(0, lower_letter)
        first_letter = "uppercase"
    
    # Check if the last char in word is a punctuation mark
    # If so, remove it
    if word[-1] in punctuation_marks:
        punctuation_mark = word.pop()
        is_punctuation_mark = True
    
    # The first letter is a consonant
    if word[0] in consonants:
        while word[0] in consonants:
            first_consonant = word.pop(0)
    ...
    # Change lowercase first letter into uppercase
    if first_letter == "uppercase":
        to_upper_letter = word.pop(0)
        upper_letter = to_upper_letter.upper()
        word.insert(0, upper_letter)
    
    # If a punctuation mark was removed from the word, 
    # add it again
    if is_punctuation_mark:
        word.append(punctuation_mark)
